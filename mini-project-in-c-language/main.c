...
int main()
{
    ...
    while (input != 5) {
        ...
        // Add book
      case 1:
            // Buffer ko clear karne ke liye (pichle scanf ka '\n' hatane ke liye)
            while ((getchar()) != '\n'); 

            printf("Enter book name = ");
            // fgets(variable, size, stdin)
            fgets(lib[i].book_name, 20, stdin);
            // fgets '\n' (Enter key) ko bhi store karta hai, use hatana zaroori hai
            lib[i].book_name[strcspn(lib[i].book_name, "\n")] = 0;

            printf("Enter author name = ");
            fgets(lib[i].author, 20, stdin);
